...
namespace Week_04_lab_12_13
{
    public class Atom
    {
        ...
        public static Atom Parse(string line)
        {
            ...
            throw new ArgumentException("string Array should have 5 elements");
            else
            {
                return new Atom(ObjectData[0], Int32.Parse(ObjectData[1]), Int32.Parse(ObjectData[2]), Double.Parse(ObjectData[3]), ObjectData[4]);
            }
        }
        public static void WriteJson(List<Atom> atoms, string filename)
        {
            TextWriter writer = new StreamWriter(filename);
            writer.Write(new JavaScriptSerializer().Serialize(atoms));
            writer.Close();
        }
        public static void ReadJson(string filename)
        {
            ...
            var jsonArray = JArray.Parse(obj["atoms"].ToString());
